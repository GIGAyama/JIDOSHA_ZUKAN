...
MODEL3D.police = {
  ...
  build: function (B) {
    ...
    /* やね（上だけ）と 左右の ほそい はしら */
    B.mesh(sideProfileGeom([
      [-0.82, 1.44], [0.42, 1.48], [0.32, 1.36], [-0.70, 1.32]
    ], W - 0.05, 0.03), paint(0xf2f5f9, { metal: 0.25, rough: 0.22, clear: 1.0 }), { x: 0, y: 0, z: 0, part: 'cab' });
    [1, -1].forEach(sz => {
      B.mesh(sideProfileGeom([[0.42, 1.48], [1.02, 1.02], [0.85, 1.02], [0.32, 1.36]], 0.07, 0.025),
        paint(0xf2f5f9, { metal: 0.25, rough: 0.22, clear: 1.0 }), { z: sz * (W / 2 - 0.065), part: 'cab' });
      B.mesh(sideProfileGeom([[-1.12, 1.00], [-0.82, 1.44], [-0.70, 1.32], [-0.92, 1.00]], 0.07, 0.025),
        paint(0xf2f5f9, { metal: 0.25, rough: 0.22, clear: 1.0 }), { z: sz * (W / 2 - 0.065), part: 'cab' });
    });
    /* 中の ゆか・ダッシュボード・ざせき */
...
function sedanBody(B, o) {
  ...
  /* やね（上だけ の 板。まえと うしろの ガラスの うらは あけて おく） */
  B.mesh(sideProfileGeom([
    [-0.82, 1.44], [0.42, 1.48], [0.32, 1.36], [-0.70, 1.32]
  ], W - 0.05, 0.03), paint(o.color, { metal: 0.28, rough: 0.20, clear: 1.0 }), { x: 0, y: 0, z: 0, part: o.part });
  /* まえと うしろの ほそい はしら（左右だけ） */
  [1, -1].forEach(sz => {
    B.mesh(sideProfileGeom([[0.42, 1.48], [1.02, 1.02], [0.85, 1.02], [0.32, 1.36]], 0.07, 0.025),
      paint(o.color, { metal: 0.28, rough: 0.20, clear: 1.0 }), { z: sz * (W / 2 - 0.065), part: o.part });
    B.mesh(sideProfileGeom([[-1.12, 1.00], [-0.82, 1.44], [-0.70, 1.32], [-0.92, 1.00]], 0.07, 0.025),
      paint(o.color, { metal: 0.28, rough: 0.20, clear: 1.0 }), { z: sz * (W / 2 - 0.065), part: o.part });
  });
